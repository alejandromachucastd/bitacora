...
function renderizarVentasImpresiones() {
    ...
    ventasImpresiones.forEach(venta => {
        ...
        if (venta.tipo === 'impresion') {
            ...
            badgeClass = 'info';
        } else if (venta.tipo === 'impresion-color') {
            tipoTexto = `Impresión Color (${venta.calidad || 'C1'})`;
            badgeClass = 'primary';
        } else {
            tipoTexto = 'Escaneo';
            badgeClass = 'warning';
...
document.getElementById('btnCorte').addEventListener('click', () => {
    ...
    // Verificar horario permitido (7:00 PM a 11:45 PM) o (10:00 AM a 2:00 PM para pruebas)
    const horarioNocturno = (hora === 19) || (hora >= 20 && hora < 23) || (hora === 23 && minutos <= 45);
    const horarioPruebas = hora >= 10 && hora < 14;
    
    if (!horarioNocturno && !horarioPruebas) {
        alert('El corte diario se puede realizar en estos horarios:\n\n🌙 Horario normal: 7:00 PM - 11:45 PM\n🔧 Horario de pruebas: 10:00 AM - 2:00 PM');
        return;
    }
