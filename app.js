...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    setInterval(verificarHorarioCorte, 60000); // Verificar cada minuto
});
 ...
 * - A las 10:00 PM: Si no hay corte del día, genera corte automáticamente
...
function verificarHorarioCorte() {
    ...
    // Si son las 10:00 PM y no se ha hecho corte, generar corte automático
    if (hora === 22 && minutos === 0) {
        ...
        if (!ultimoCorte || ultimoCorte.fecha !== hoy) {
            alert('⏰ Son las 10:00 PM. Generando corte automático...');
...
function agregarServicioExtraModal(event) {
    ...
    mostrarToast('Servicio Agregado', `${servicio.descripcion} - $${servicio.precio.toFixed(2)}`, 'success');
...
function ventaRapidaTramite() {
    ...
    mostrarToast('Venta Registrada', `${tramite.nombre} x${cantidad} - $${total.toFixed(2)}`, 'success');
}
...
function renderizarVentasImpresiones() {
    ...
    ventasImpresiones.forEach(venta => {
        const equipo = venta.equipoId ? equipos.find(e => e.id === venta.equipoId) : null;
        
        const tr = document.createElement('tr');
        tr.innerHTML = `
            <td><span class="badge ${venta.tipo === 'impresion' ? 'info' : 'warning'}">${venta.tipo === 'impresion' ? 'Impresión' : 'Escaneo'}</span></td>
...
function calcularPrecioImpresion(cantidad) {
    ...
    return cantidad * 1; // Precio base $1
}
...
document.getElementById('btnCorte').addEventListener('click', () => {
    ...
    // Verificar horario permitido (8:00 PM a 12:00 AM) o (10:00 AM a 2:00 PM para pruebas)
    const horarioNocturno = hora >= 20 && hora < 24;
    const horarioPruebas = hora >= 10 && hora < 14;
    
    if (!horarioNocturno && !horarioPruebas) {
        alert('El corte diario se puede realizar en estos horarios:\n\n🌙 Horario normal: 8:00 PM - 12:00 AM\n🔧 Horario de pruebas: 10:00 AM - 2:00 PM');
        return;
    }
...
function generarCorteAutomatico() {
    ...
    console.log('✅ Corte automático generado a las 10 PM');
}
...
function exportarPDF() {
    ...
    if (!horarioNocturno && !horarioPruebas) {
        alert('Los PDFs se pueden exportar en estos horarios:\n\n🌙 Horario normal: 8:00 PM - 12:00 AM\n🔧 Horario de pruebas: 10:00 AM - 2:00 PM');
        return;
    }
...
const registrarVentaTramiteOriginal = registrarVentaTramite;
window.registrarVentaTramite = function(event) {
    registrarVentaTramiteOriginal(event);
    const tramiteId = document.getElementById('ventaTramiteSelect').value;
    const tramite = tramites.find(t => t.id === tramiteId);
    mostrarToast('Venta Registrada', `${tramite.nombre} vendido exitosamente`, 'success');
};
