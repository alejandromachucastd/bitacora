...
async function inicializarApp() {
    ...
    // Cargar cambios diarios desde Firebase
    await cargarCambiosDiarios();
...
function renderizarReportes() {
    ...
    // Calcular y mostrar totales por mes
    const totalPorMes = calcularTotalPorMes();
    mostrarTotalesPorMes(totalPorMes);
    
    // Mostrar total general de todos los cortes
    mostrarTotalGeneralCortes();
    
    // Verificar y actualizar estado del cambio diario
    verificarEstadoCambio();
...
// ============================================
// TOTALES POR MES Y GENERAL
// ============================================

/**
 * Calcula el total acumulado por mes de todos los cortes
 * @returns {Object} Objeto con totales por mes
 */
function calcularTotalPorMes() {
    const totalesPorMes = {};

    cortesHistorial.forEach(corte => {
        ...
        totalesPorMes[mes].totalNeto += corte.totalNeto;
        totalesPorMes[mes].cortes++;
    });
...
/**
 * Muestra los totales acumulados por mes en la interfaz
 * @param {Object} totalesPorMes - Objeto con totales por mes
 */
function mostrarTotalesPorMes(totalesPorMes) {
    const container = document.getElementById('totalesPorMes');
    if (!container) return;
    ...
    container.innerHTML = `
        <div style="background: var(--bg-tertiary); padding: 1.5rem; border-radius: 8px; margin-top: 1rem;">
            <h4 style="margin: 0 0 1rem 0; color: var(--accent-primary);">📊 Total Acumulado por Mes</h4>
            ${mesesOrdenados.length > 0 ?
                mesesOrdenados.map(mes => {
                    const datos = totalesPorMes[mes];
                    const nombreMes = new Date(mes + '-01').toLocaleDateString('es-MX', { year: 'numeric', month: 'long' });
                    return `
                        <div style="display: flex; justify-content: space-between; align-items: center; padding: 0.5rem 0; border-bottom: 1px solid var(--border);">
                            <span><strong>${nombreMes}</strong></span>
                            <div style="text-align: right;">
                                <span style="font-size: 1.1rem; color: var(--success); font-weight: 600;">
                                    $${datos.totalNeto.toFixed(2)}
                                </span>
                                <br>
                                <small style="color: var(--text-secondary);">${datos.cortes} corte${datos.cortes !== 1 ? 's' : ''}</small>
                            </div>
                        </div>
                    `;
                }).join('') :
                '<p style="color: var(--text-secondary); text-align: center; margin: 1rem 0;">No hay cortes registrados</p>'
            }
        </div>
    `;
}
...
/**
 * Calcula el total general de todos los cortes
 * @returns {Object} Total general y cantidad de cortes
 */
function calcularTotalGeneralCortes() {
    let totalGeneral = 0;
    let cantidadCortes = 0;

    cortesHistorial.forEach(corte => {
        totalGeneral += corte.totalNeto;
        cantidadCortes++;
    });

    return { totalGeneral, cantidadCortes };
}

/**
 * Muestra el total general de todos los cortes en la interfaz
 */
function mostrarTotalGeneralCortes() {
    const { totalGeneral, cantidadCortes } = calcularTotalGeneralCortes();
    
    const totalElement = document.getElementById('totalGeneralCortes');
    const cantidadElement = document.getElementById('cantidadTotalCortes');
    ...
    if (cantidadElement) {
        cantidadElement.textContent = cantidadCortes;
    }
}

// ============================================
// SISTEMA DE CAMBIO DIARIO
// ============================================

/**
 * Carga los cambios diarios desde Firebase (solo Firebase, no localStorage)
 * @returns {Promise<Array>}
 */
async function cargarCambiosDiarios() {
    if (!useLocalStorage && db) {
        try {
            const snapshot = await db.ref('cambiosDiarios').once('value');
            const cambiosFirebase = snapshot.val();
            
            if (cambiosFirebase) {
                // Convertir objeto de Firebase a array
                const cambiosArray = Object.values(cambiosFirebase);
                
                // Ordenar por timestamp
                cambiosArray.sort((a, b) => a.timestamp - b.timestamp);
                
                console.log(`✅ ${cambiosArray.length} cambios cargados desde Firebase`);
                return cambiosArray;
            } else {
                console.log('ℹ️ No hay cambios en Firebase');
                return [];
            }
        } catch (error) {
            console.error('❌ Error al cargar cambios desde Firebase:', error);
            return [];
        }
    } else {
...
/**
 * Obtiene los cambios desde Firebase en tiempo real
 * @returns {Promise<Array>}
 */
async function obtenerCambiosDesdeFirebase() {
    if (!useLocalStorage && db) {
        try {
            const snapshot = await db.ref('cambiosDiarios').once('value');
            const cambiosFirebase = snapshot.val();
...
/**
 * Verifica si el horario actual permite agregar cambio (7 AM - 9 AM)
 * @returns {boolean} True si está en horario permitido
 */
function verificarHorarioCambio() {
    const now = new Date();
    const hora = now.getHours();
...
/**
 * Verifica si ya se agregó el cambio hoy (desde Firebase)
 * @returns {Promise<boolean>} True si ya se agregó el cambio hoy
 */
async function yaSeAgregoCambioHoy() {
    const cambios = await obtenerCambiosDesdeFirebase();
    const hoy = new Date().toLocaleDateString('es-MX');
    
    return cambios.some(cambio => cambio.fecha === hoy);
}

/**
 * Obtiene el cambio del día actual o $100 por defecto
 * @returns {Promise<number>} Monto del cambio
 */
async function obtenerCambioDelDia() {
    const cambios = await obtenerCambiosDesdeFirebase();
    const hoy = new Date().toLocaleDateString('es-MX');
    
    const cambioHoy = cambios.find(c => c.fecha === hoy);
    
    // Si no hay cambio registrado, retornar $100 por defecto
    return cambioHoy ? cambioHoy.monto : 100;
}

/**
 * Verifica si está cerca del horario límite (8:50 AM)
 * @returns {boolean} True si está entre 8:50 AM y 9:00 AM
 */
function estaCercaDelLimite() {
    const now = new Date();
    const hora = now.getHours();
    const minutos = now.getMinutes();
    return hora === 8 && minutos >= 50;
}

/**
 * Muestra notificación de recordatorio si está cerca del límite
 */
async function verificarNotificacionLimite() {
    const yaAgregado = await yaSeAgregoCambioHoy();
    const cercaDelLimite = estaCercaDelLimite();
    const yaNotificado = sessionStorage.getItem('notificacionCambioMostrada');
    
    if (!yaAgregado && cercaDelLimite && !yaNotificado) {
        mostrarToast(
            '⏰ Recordatorio',
            '¡Quedan menos de 10 minutos para agregar el cambio del día!',
            'warning'
        );
        sessionStorage.setItem('notificacionCambioMostrada', 'true');
    }
}

/**
 * Verifica y actualiza el estado del cambio diario en la UI
 */
async function verificarEstadoCambio() {
    // Actualizar en Reportes
    const btnAgregar = document.getElementById('btnAgregarCambio');
    const cambioEstado = document.getElementById('cambioEstado');
    const cambioRegistrado = document.getElementById('cambioRegistrado');
    
    // Actualizar en Dashboard
    const btnAgregarDash = document.getElementById('btnAgregarCambioDashboard');
    
    const yaAgregado = await yaSeAgregoCambioHoy();
    
    // Verificar si debe mostrar notificación de límite
    await verificarNotificacionLimite();
    
    if (yaAgregado) {
        // Ya se agregó el cambio hoy
        const cambios = await obtenerCambiosDesdeFirebase();
        const hoy = new Date().toLocaleDateString('es-MX');
        const cambioHoy = cambios.find(c => c.fecha === hoy);
        
        // Actualizar Reportes
        if (btnAgregar && cambioEstado && cambioRegistrado) {
            btnAgregar.style.display = 'none';
            cambioEstado.style.display = 'none';
            cambioRegistrado.style.display = 'block';
            
            if (cambioHoy) {
                document.getElementById('montoCambioHoy').textContent = `$${cambioHoy.monto.toFixed(2)}`;
                document.getElementById('horaCambio').textContent = `Registrado a las ${cambioHoy.hora}`;
            } else {
                // Mostrar $100 por defecto
                document.getElementById('montoCambioHoy').textContent = `$100.00`;
                document.getElementById('horaCambio').textContent = `Cambio por defecto (no registrado)`;
            }
        }
        
        // Actualizar Dashboard - Ocultar botón si ya se agregó
        if (btnAgregarDash) {
            btnAgregarDash.style.display = 'none';
        }
        
        const montoDash = document.getElementById('montoCambioHoyDashboard');
        const horaDash = document.getElementById('horaCambioDashboard');
        
        if (montoDash) {
            if (cambioHoy) {
                montoDash.textContent = cambioHoy.monto.toFixed(2);
                if (horaDash) {
                    horaDash.textContent = cambioHoy.hora;
                    horaDash.style.display = 'inline';
                }
            } else {
                // Mostrar $100 por defecto
                montoDash.textContent = '100.00';
                if (horaDash) {
                    horaDash.textContent = 'por defecto';
                    horaDash.style.display = 'inline';
                }
            }
        }
    } else {
        // No se ha agregado el cambio - MOSTRAR BOTÓN SIEMPRE
        if (btnAgregar && cambioEstado && cambioRegistrado) {
            btnAgregar.style.display = 'inline-block';
            cambioEstado.innerHTML = '<p style="color: var(--text-secondary);">El cambio se puede registrar entre las 7:00 AM y 9:00 AM</p>';
            cambioRegistrado.style.display = 'none';
        }
        
        // Mostrar botón en Dashboard SIEMPRE
        if (btnAgregarDash) {
            btnAgregarDash.style.display = 'inline-block';
        }
...
/**
 * Muestra el modal para agregar cambio diario
 */
async function mostrarModalCambio() {
    if (!verificarHorarioCambio()) {
        alert('⏰ El cambio solo se puede agregar entre las 7:00 AM y 9:00 AM');
        return;
    }
    
    const yaAgregado = await yaSeAgregoCambioHoy();
    if (yaAgregado) {
        alert('✅ Ya se agregó el cambio del día hoy');
        return;
    }
...
/**
 * Guarda el cambio diario SOLO en Firebase (no localStorage)
 * @param {Event} event - Evento del formulario
 */
async function guardarCambio(event) {
    ...
    if (monto < 100) {
        alert('❌ El monto mínimo de cambio es de $100.00');
        return;
    }
    
    if (monto > 200) {
        alert('❌ El monto máximo de cambio es de $200.00');
        return;
    }
    ...
    if (!useLocalStorage && db) {
        try {
            await db.ref('cambiosDiarios/' + id).set(nuevoCambio);
            console.log('✅ Cambio guardado en Firebase');
            
            cerrarModal('modalCambio');
            mostrarToast('Cambio Registrado', `Se registró el cambio de $${monto.toFixed(2)}`, 'success');
            
            // Actualizar la UI
            await verificarEstadoCambio();
            await mostrarHistorialCambios();
        } catch (error) {
            console.error('❌ Error al guardar en Firebase:', error);
            cerrarModal('modalCambio');
            mostrarToast('Error', 'No se pudo guardar el cambio en Firebase', 'error');
        }
    } else {
        cerrarModal('modalCambio');
        mostrarToast('Error', 'Firebase no está disponible', 'error');
    }
}

/**
 * Muestra el historial de cambios de los últimos 7 días (desde Firebase)
 */
async function mostrarHistorialCambios() {
    const container = document.getElementById('historialCambios');
    if (!container) return;
    
    const cambios = await obtenerCambiosDesdeFirebase();
    
    // Obtener los últimos 7 días
    const ultimos7Dias = cambios.slice(-7).reverse();
    
    if (ultimos7Dias.length === 0) {
        container.innerHTML = '<p style="color: var(--text-secondary); text-align: center; padding: 1rem;">No hay cambios registrados</p>';
        return;
    }
    
    container.innerHTML = ultimos7Dias.map(cambio => `
        <div style="display: flex; justify-content: space-between; align-items: center; padding: 0.75rem; background: var(--bg-secondary); border-radius: 6px; margin-bottom: 0.5rem; border-left: 3px solid var(--success);">
            <div>
                <strong style="color: var(--text-primary);">${cambio.fecha}</strong>
                <br>
                <small style="color: var(--text-secondary);">Registrado a las ${cambio.hora}</small>
            </div>
            <div style="text-align: right;">
                <strong style="color: var(--success); font-size: 1.2rem;">$${cambio.monto.toFixed(2)}</strong>
            </div>
        </div>
    `).join('');
}
